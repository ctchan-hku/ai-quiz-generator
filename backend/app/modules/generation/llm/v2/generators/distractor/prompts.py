...
_MIN_WRONG = MC_QUESTION_OPTION_COUNT_MIN - 1
_MAX_WRONG = MC_QUESTION_OPTION_COUNT_MAX - 1
_DEFAULT_WRONG = MC_QUESTION_OPTION_COUNT_DEFAULT - 1
_TOTAL_DEFAULT = MC_QUESTION_OPTION_COUNT_DEFAULT

USER_PROMPT = (
    "For each numbered block above, output exactly one object in `items` in the same order.\n"
    "There must be exactly {num_questions} entries in `items`.\n"
    f"Each object's `distractors` must contain between {_MIN_WRONG} and {_MAX_WRONG} "
    ...
    f"Default target: exactly {_DEFAULT_WRONG} distractors "
    f"({_TOTAL_DEFAULT} options total including the correct answer). "
