...
def _gear_access_token(secret: str, *, user_id: str, expired: bool = False) -> str:
...
def test_resolve_user_id_from_gear_access_token() -> None:
    service = AccessTokenService(secret="test-access-secret")
    token = _gear_access_token("test-access-secret", user_id="507f1f77bcf86cd799439011")
...
def test_resolve_rejects_wrong_secret() -> None:
    token = _gear_access_token("secret-a", user_id="user-1")
